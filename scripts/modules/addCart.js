...
const addCart = (target, text, selectorText) => {
  let obj = {};
  const cart = getStorage('cart');
  const findItem = cart.find(item => item.id === target.dataset.id);
  if (findItem) {
    obj = findItem;
    obj.count += 1;
  } else {
    obj.count = 1;
    obj.id = target.dataset.id;
    cart.push(obj);
  }

  if (text) {
    target.textContent = text.replace('{count}', obj.count);
  }
